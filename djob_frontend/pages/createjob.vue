...
let { data: jobCategories } = useFetch('http://localhost:8000/api/v1/jobs/categories')
...
async function submitForm() {
    ...
    if (position_salary.value == '') { errors.value.push('The position_salary field is missing')}
    if (position_location.value == '') { errors.value.push('The position_location field is missing')}
    if (company_name.value == '') { errors.value.push('The company_name field is missing')}
    if (company_location.value == '') { errors.value.push('The company_location field is missing')}
    if (company_email.value == '') { errors.value.push('The company_email field is missing')}
...
<template>
    <div class="py-10 px-6">
        ...
        <form v-on:submit.prevent="submitForm" class="space-y-4">
            ...
            <button class="py-4 px-6 mt-2 bg-teal-700 text-white rounded-xl">Submit</button>
